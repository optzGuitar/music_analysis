...
class SlidingWindow(Incremental):
    def __init__(self,
        ...
        super().__init__(range, random_heuristics=random_heuristics, composer_files=composer_files,
                         parallel_mode=parallel_mode, key=key, add_pattern_incrementally=add_pattern_incrementally)
        self._rule_selector_service = SlidingWindowRuleSelectorService()
        self._max_window_size = max_window_size

    # TODO: override basic atoms
    ...
    def ground(self, from_timestep: int, to_timestep: int):
        ...
        rules = self._rule_selector_service.select(to_timestep)
        rules += self._models_per_length[max_len][-1] \
            .get_notes() if max_len is not None else []
